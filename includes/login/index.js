...
function buildAPI(globalOptions, html, token, jar) {
  ...
  if (!i_user && !c_user) {
    throw { error: "Erreur, Impossible de se connecter au compte du Bot il se pourrait que le compte soit bloqué,Ou veuillez changer d'anderstate." };
  }
  ...
  console.log(co("[ BASE DE DONNÉ ]"), (cra("[ CONNEXION ]")), `Connexion sur ${userID}`);
  ...
  if (token == 'NONE') 
    ...
    console.log(co("[ BASE DE DONNÉ ]"), (cra("[ CONNEXION ]")), ``);
  if (oldFBMQTTMatch) {
    irisSeqID = oldFBMQTTMatch[1];
    mqttEndpoint = oldFBMQTTMatch[2];
    region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
    console.log(co("[ BASE DE DONNÉE ]"), (cra("[ CONNEXION ]")), `Région: ${region}`);
  } else {
    ...
    if (newFBMQTTMatch) {
      ...
      if (legacyFBMQTTMatch) {
        ...
        region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
      console.log(co("[ BASE DE DONNÉE ]"), (cra("[ CONNEXION ]")), `Votre compte est déconnecter `);
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
  return function(res) {
    ...
    console.log(co("[ CONNEXION ]"), (chalk.yellow(' ')), `Ander en connexion sur...`);
    return utils
      ...
      .then(function(res) {
        var headers = res.headers;
         if (!headers.location)
          // return;
         throw { error: "erreur Email/Mot de passe." };

        // This means the account has login approvals turned on.
        if (headers.location.includes('.com/checkpoint/')) {
          //log.info("login", "You have login approvals turned on.");
          console.log(co("[ CONNEXION ]"), (chalk.yellow(' ')), `VEUILLEZ DÉSACTIVER LE FACTEUR À DEUX.'`);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function() {
      console.log(co("[ BASE DE DONNÉE ]"), (cra("[ CONNEXION ]")), 'Connexion effectuer.'); return callback(null, api);
    })
    .catch(function(e) {
